...
set -eu
# Script settings 
RELEASE=$(lsb_release -c -s) # focal or bionic
NEAR_VERSION="1.18.3"
...
function create_validator_container {
    ...
    then
    echo "* Launching Ubuntu $RELEASE Cloud Image"
    lxc launch images:ubuntu/focal/cloud/amd64 ${validator_name}
    prepare_validator
    fi
    ...
    then
    echo "* Launching Ubuntu $RELEASE Cloud Image"
    lxc launch images:ubuntu/18.04/cloud/amd64 ${validator_name}
    prepare_validator
    fi
}
...
function create_neard_service {
    ...
    ")
    lxc exec ${validator_name} -- sh -c "echo $SERVICE > /etc/systemd/system/neard.service"
